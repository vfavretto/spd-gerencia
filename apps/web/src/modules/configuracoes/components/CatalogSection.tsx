...
import type { CatalogItem, SectionConfig } from "@/modules/configuracoes/types";
...
export function CatalogSection({ section, data = [] }: CatalogSectionProps) {
  ...
  useEffect(() => {
    ...
    reset(defaultValues);
  }, [defaultValues, editing, reset, section.fields]);
    ...
    <section className="glass-panel flex flex-col gap-4 p-6">
      ...
      {isAdmin && (
        <form className="grid gap-4" onSubmit={handleSubmit(onSubmit)}>
            ...
            {section.fields.map((field) => (
              <div key={field.name} className={field.textarea ? "md:col-span-2" : ""}>
                <label className="form-label">
                  {field.label}
                  {editing && field.required && <span className="text-xs text-rose-500"> *</span>}
                </label>
                {field.textarea ? (
                  <textarea
                    rows={3}
          ...
          {mutation.isError && (
            <p className="text-sm text-rose-600">✗ Erro ao salvar. Verifique os dados e tente novamente.</p>
          )}
        </form>
      )}
      ...
      <div className="overflow-x-auto rounded-3xl border border-slate-100">
        ...
        {data.length === 0 && (
          <p className="p-6 text-center text-sm text-slate-400">Nenhum registro cadastrado.</p>
        )}
      </div>
    </section>
  );
